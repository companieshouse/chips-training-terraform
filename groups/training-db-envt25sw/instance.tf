resource "aws_instance" "training_db_envt25sw" {
  ...
  key_name      = aws_key_pair.training_db_envt25sw.key_name
  
  iam_instance_profile   = module.instance_profile.aws_iam_instance_profile.name
  vpc_security_group_ids = [aws_security_group.training_db_envt25sw.id]
  tags = {
...
resource "aws_ebs_volume" "ora1" {
  availability_zone = aws_instance.training_db_envt25sw[0].availability_zone
...
resource "aws_key_pair" "training_db_envt25sw" {
 key_name   = "${local.common_resource_name}-public-key"
 public_key = local.training_db_envt25sw_public_key
}
